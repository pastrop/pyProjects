...
class WebCrawler(object):
    ...
    def crawl(self, initial_url, output_file, display_results=True):
        ...
        for k,v in self.graph.nodes.items():
            print('print in crawl key-{}::value-{}'.format(k,v))        
        #print('print in crawl***********', self.graph.nodes)
    ...
    def _enqueue(self, url):
        if url in self.graph.nodes:
            return
            # printing
        print('inside _enqueue: url', url)    
        self.graph.add_node(url)
        #printing
        print('inside _enqueue: graph.node',self.graph[url] )
